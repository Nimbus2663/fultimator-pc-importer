...
import { applyOverrides, convertFultimatorPC, parseFultimatorInput } from "./converter048.js";

const ULTIMATE_STORY_KEY = "ultimate.story.extension/metadata";
const COMPENDIUM_SEND_KEY = "ultimate.story.extension/sendskill";
const COMPENDIUM_CACHE_KEY = "fultimator.pc.importer/compendiumCache";
...
async function boot() {
  ...
  try {
    await OBR.onReady(async () => {
      ...
      await refreshCompendiumCache();
      updateSceneStatus();
      if (state.sceneReady) {
        await refreshSceneCharacters();
      }
      OBR.room.onMetadataChange((metadata) => {
        captureCompendiumSkill(metadata);
      });
...
function parseAndPreview() {
  try {
    ...
    state.converted = state.sources.map((source) => convertFultimatorPC(source, {
      ...
      compendium: state.compendiumCache
    }));
...
async function refreshCompendiumCache() {
  try {
    const metadata = await OBR.room.getMetadata();
    state.compendiumCache = metadata?.[COMPENDIUM_CACHE_KEY] && typeof metadata[COMPENDIUM_CACHE_KEY] === "object"
      ? { ...metadata[COMPENDIUM_CACHE_KEY] }
      : {};
    await captureCompendiumSkill(metadata, false);
  } catch {
    state.compendiumCache = {};
  }
}

async function captureCompendiumSkill(metadata, persist = true) {
  const sent = metadata?.[COMPENDIUM_SEND_KEY];
  if (!sent || typeof sent !== "object") return;
  const name = sent.skillName || sent.name;
  const detail = sent.detail || sent.description;
  ...
  const nextEntry = {
    name: cleanText(name),
    info: cleanText(sent.info || ""),
    description: cleanText(detail)
  };
  const previous = state.compendiumCache[key];
  if (previous?.description === nextEntry.description && previous?.info === nextEntry.info) return;
  ...
  if (persist) {
    try {
      await OBR.room.setMetadata({ [COMPENDIUM_CACHE_KEY]: state.compendiumCache });
    } catch {
      // The in-memory cache still helps this open import panel.
    }
  }
